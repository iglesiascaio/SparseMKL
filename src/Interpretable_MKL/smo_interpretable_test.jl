module InterpretableMKL
    ...
    ################################################################################
    #                 S P A R S E   B E T A   O P T I M I Z A T I O N
    ################################################################################
    ...
    function sparse_optimize_beta(K_list, α, y, λ, k0)
        ...
        # Optional: normalize β to avoid numerical issues
        s = sum(β)
        if s > 0
            β ./= s
        else
    ...
    ################################################################################
    #             C O M P U T E   T H E   D U A L   O B J E C T I V E
    ################################################################################

    function compute_objective(α, y, K, β)
        """
        Dual objective: sum(α) - 0.5 * Σᵢⱼ (αᵢ αⱼ yᵢ yⱼ K[i,j])
        """
        return sum(α) - 0.5 * sum(y[i]*y[j]*α[i]*α[j]*K[i,j]
                                for i in eachindex(y), j in eachindex(y))
    end

    ################################################################################
    #           S E Q U E N T I A L   M I N I M A L   O P T I M I Z A T I O N (SMO)
    ################################################################################
    ...
    ################################################################################
    #    M A I N   I N T E R P R E T A B L E   M K L   T R A I N I N G   L O O P
    ################################################################################
    ...
    function train_interpretable_mkl(
        ...
        for iter in 1:max_iter
            ...
            # === 1) Solve for α via SMO (instead of Gurobi) ===
            α, b = train_svm_smo!(K_combined, y, C;
                                tol=tolerance,
                                eps=1e-3,
                                max_passes=5)

            # === 2) Optimize β given α ===
            ...
            # Recompute combined kernel
            K_combined = compute_combined_kernel(K_list, β)


            # Check for convergence in β
            if iter > 1 && norm(β - β_old) < tolerance
                println("Converged after $iter iterations.")
                println("Final β: ", β)
                println("Final objective function: ",
                    compute_objective(α, y, K_combined, β))
                break
            end
            ...
            println("Difference in β: ", norm(β - β_old))
